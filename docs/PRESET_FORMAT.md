...
- `irCut`: physics-based IR-cut filter removal (the DSC-V1 magnetic bypass), the very first module on the physics rail — it changes what light reaches the sensor before charge, voltage, or bits exist. A near-infrared plane is synthesized from the raw Bayer quads (red-weighted luminance, so blue skies stay dark; a Wood-effect term makes chlorophyll-green foliage glow near-white regardless of visible brightness), blurred for sensor halation, and leaked into every color well with auto-exposure pulling the level back down. `strength`: how far the filter is pulled (0 = seated, 1 = removed). `spectrum`: how unevenly the color dyes pass IR — low is deep NIR (every channel sees it: ghost-white monochrome world), high leaks mostly into red (the pink full-spectrum cast). `wood`: foliage NIR reflectance boost. `haze`: halation blur reach (IR focuses behind the sensor plane). `wbRed`/`wbBlue` as in `busBend` — full-spectrum shooters set strange custom white balances, and low red gains tip the develop toward aerochrome-ish palettes.
...
- `afeBend`: physics-based analog-front-end bend, second on the physics rail (after `ccdClock`, before `railSag` and `busBend`; the rail shares one raw round trip, see below). Attacks the pixel voltage stream before the ADC: `inject` adds an oscillator into the signal (`wave`: `sine`/`square`/`saw`/`noise`; `freq` is a log sweep in cycles per sensor row, ~0.02-320 — low = rolling horizontal hum bands, high = pixel-clock moire; `skew` tilts the bands by adding phase per row; `wobble` is seeded phase drift). `gainMod` modulates analog gain / ADC reference with the same oscillator (pumping exposure instead of additive bands). `cdsAmount`/`cdsSkew` skew the correlated-double-sampling timing so the reset sample lands on a stale pixel (log 1-48 clocks back): the frame becomes an embossed horizontal derivative with negative trails. `wbRed`/`wbBlue` as in `busBend`. When several physics modules are enabled they share a single inverse/forward ISP pass in signal order (charge -> analog -> supply rail -> bus -> capture framing), with WB gains taken from the first enabled module.
- `masterClock`: physics-based master-clock reclock bend (the LTC1799 mod), last on the physics rail — the DSP captures the whole (already bent) pixel stream at the wrong rate. One continuous sampling position walks the stream at a detuned ratio: content stretches or compresses along the scan, leftover line phase accumulates into skew and then full vertical roll, and an H-sync PLL with a finite capture range pulls lines back when it can (catch-and-roll tears emerge on their own). `detune` (-0.5..0.5): oscillator error, negative underclocks, positive overclocks — a few percent is VHS-ish warble, the ends of the range are total sync collapse; `drift`: slow seeded wobble of the rate (wavy tape skew); `hLock`: PLL capture strength (high = held picture with snap tears, low = free roll); `shred`: fraction of rows whose sample clock slips at pixel level, capturing the Bayer mosaic out of phase — colored line static dosed independently of the geometry. Ratio error and PLL dynamics are normalized to frame height so roll and skew match across resolutions. `wbRed`/`wbBlue` as in `busBend`.
- `addressBus`: physics-based SDRAM address-line bend, last on the physics rail (the frame buffer holds the stream the DSP already captured). Stuck, bridged, or swapped address lines alias memory reads in exact powers of two: mirrored tiles, interleaved row pairs, ping-ponged frame halves — crystalline deterministic repetition, nothing like `memoryFault`'s painterly block damage. `rows`/`cols`: how many row / column address lines are damaged (1-3 seeded faults per axis). `scale`: which lines the faults land on — low bits give fine pixel-pair interleaves, high bits fold whole halves; positions are normalized to the frame dimension so tile size tracks resolution. `lowBit`: chance a fault hits address line 0, the only one that breaks Bayer parity — aliased regions develop color-swapped (dosed separately, like `masterClock.shred`). `duty`: fraction of the readout the marginal contact is actually failing — 1 is a solid short folding the whole frame, lower values gate the fault in and out in smoothed bands of rows (the documented horizontal-tearing look). `wbRed`/`wbBlue` as in `busBend`.
