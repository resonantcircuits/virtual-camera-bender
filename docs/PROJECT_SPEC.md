...
**2. `masterClock` — system-clock reclock bend (timing domain) — done 2026-07-08**
...
Shipped as `src/master-clock.js`, *last* on the rail (the DSP re-frames the already-bent stream — this is what "warps the timing of all of it" means in practice). Implementation notes: one continuous sampling position at ratio `1+err` per output pixel; the H-sync PLL sees only line phase (error mod one line) with a finite capture range, so vertical roll survives lock and catch-and-roll tears emerge unscripted; sampling offsets snap to CFA parity so geometry warps without color damage — per-sample Bayer slip is dosed separately by a `shred` knob (rows captured out of mosaic phase develop into colored line static, the documented "heavy color-phase drift"). Detune taper is superlinear so warble → rolling bands → venetian → collapse spread across the knob; ratio error and PLL dynamics normalized to a 960-row frame. Validated: idle bit-identical, band geometry matches 720px↔6240px, video clean. Built-in presets: `Loose Sync`, `Reclock Roll` (48 total).

**3. `busBend` v2 — patch bay, soft shorts, corrosion — (a)+(b) done 2026-07-08**
